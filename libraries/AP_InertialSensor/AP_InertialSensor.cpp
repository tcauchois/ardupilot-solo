...
//gyro init variables
static uint8_t num_gyros, num_converged, sample_cnt;
static uint16_t prev_time, _test_step;
static Vector3f last_average[INS_MAX_INSTANCES], best_avg[INS_MAX_INSTANCES];
static Vector3f new_gyro_offset[INS_MAX_INSTANCES];
static float best_diff[INS_MAX_INSTANCES];
static bool converged[INS_MAX_INSTANCES];
static Vector3f gyro_sum[INS_MAX_INSTANCES], gyro_avg[INS_MAX_INSTANCES], gyro_diff[INS_MAX_INSTANCES];
static Vector3f accel_start;
static Vector3f accel_diff;
static float diff_norm[INS_MAX_INSTANCES];
static enum Rotation saved_orientation;
    ...
    switch(_init_state){
        case INITIAL:
            ...
            /*
              ...
              having to rotate readings during the calibration
            */
            saved_orientation = _board_orientation;
            _board_orientation = ROTATION_NONE;
            ...
            num_converged = 0;
            if(!blocking){
                _init_state = COLLECTION;
            } else{
                _init_state = BLOCKING_TEST_STABLE;
            }
            return;
        case COLLECTION:
            if((prev_time - hal.scheduler->millis()) > 500){
                _collection();
            }
            prev_time = hal.scheduler->millis();
            if(sample_cnt == 50){
                sample_cnt = 0;
                _init_state = TEST_STABLE;
            }
            return;
        case TEST_STABLE:
            _test_stable();
            if(_test_step <= 30*4 && num_converged < num_gyros){
                _test_step++;
                _init_state = COLLECTION;
            } else{
                _init_state = COMPLETE;
            }
            return;
        case BLOCKING_TEST_STABLE:
            for(int16_t j = 0; j <= 30*4 && num_converged < num_gyros; j++){
                for(sample_cnt = 0; sample_cnt<50; sample_cnt++){
                    _collection();
                }
                _test_stable();
            }
...
void 
AP_InertialSensor::_test_stable(){
    accel_diff = get_accel(0) - accel_start;
    ...
    for (uint8_t k=0; k<num_gyros; k++) {
        gyro_avg[k] = gyro_sum[k] / sample_cnt;
